...
openHunt=function(){
 ...
 limit.innerHTML='AI research per day<input name="aiDailyLimit" type="number" min="10" max="500" value="'+sourcingPolicy().aiDailyLimit+'" required><span class="mp-field-help">Maximum requests Marketplace AI may use for new Marketplace research.</span>';advanced?.append(limit);
...
function automationIssue(side){
 ...
 if(blocker&&/companion|Chrome|connect.*extension|socket|sign.?in|log.?in|CAPTCHA|verif/i.test(note))return {side,title:'Browser connection needs attention',detail:'Open Connections, restore Chrome access, then Marketplace AI will retry the waiting work.',page:'connections',target:'[data-provider-card="Chrome"]'};
