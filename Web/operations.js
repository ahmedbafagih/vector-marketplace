...
openHunt=function(){
 ...
 form.onsubmit=e=>{const aiDailyLimit=Number(form.elements.aiDailyLimit.value);if(!Number.isInteger(aiDailyLimit)||aiDailyLimit<10||aiDailyLimit>500){e.preventDefault();feedback('Choose 10 to 500 daily sourcing AI requests.');return}auto.sourcing={...sourcingPolicy(),aiDailyLimit};submit(e);if(!settings){openHunt();const saved=$('#vx-source-save-state');saved.textContent='Saved just now';saved.dataset.saved='true';requestAnimationFrame(()=>{if(typeof saved.scrollIntoView==='function')saved.scrollIntoView({block:'center',behavior:'auto'})});feedback('Buying settings saved. Daily AI requests: '+aiDailyLimit+'.')}}
};
