...
const VECTOR_COMPANION_STORE_URL='https://chromewebstore.google.com/detail/vector-marketplace-companion/cfkdpkejcgfgokangnmkoofnmigkkokg';
...
function vectorSetupSteps(){return [
 {eyebrow:'WELCOME',title:'Meet Vector',copy:'Set your buying rules once. Vector will search, review, and keep your Marketplace work organized on this Mac.'},
 {eyebrow:'1 OF 5',title:'What should Vector find?',copy:'Use exact models for precise results, or broader terms when you want more variety.'},
 {eyebrow:'2 OF 5',title:'How far will you travel?',copy:'Your starting point lets Vector calculate real pickup distance when a seller shares a location.'},
 {eyebrow:'3 OF 5',title:'What makes a deal worthwhile?',copy:'These limits keep purchases inside your budget and profit target.'},
 {eyebrow:'4 OF 5',title:'How much can Vector do?',copy:'Start with review only, or let Vector contact sellers when every rule passes.'},
 {eyebrow:'5 OF 5',title:'Connect the two services',copy:'Vector needs your AI runtime and the Chrome companion before it can search Marketplace.'},
...
function vectorSetupBody(step){const d=vectorOnboarding;
 if(step===0)return '<div class="vx-onboarding-hero" aria-hidden="true"><span></span><span></span><span></span><i></i></div><ul class="vx-onboarding-benefits"><li><strong>Local by default</strong><span>Your workspace stays on this Mac.</span></li><li><strong>Rules before outreach</strong><span>Vector contacts sellers only inside your limits.</span></li><li><strong>About 2 minutes</strong><span>Five short decisions, then a connection check.</span></li></ul>';
 if(step===1)return '<label class="vx-onboarding-field">Items or models<textarea name="targets" rows="5" maxlength="1200" placeholder="iPhone 11\nHerman Miller Aeron" autofocus>'+esc(d.targets)+'</textarea><span>One per line, up to 20. Vector also rotates singular and plural searches.</span></label><div class="vx-onboarding-tip"><strong>Be specific when it matters</strong><span>“iPhone 11” avoids unrelated iPhones. “Vintage furniture” explores a broader category.</span></div>';
...
function renderVectorOnboarding(){
 ...
 el.innerHTML='<div class="vx-onboarding-shell"><aside class="vx-onboarding-side"><span class="mp-orbit" aria-hidden="true"></span><strong>VECTOR</strong><ol>'+steps.slice(1,6).map((_,i)=>'<li data-state="'+(i+1<step?'done':i+1===step?'active':'upcoming')+'"><span>'+(i+1<step?'✓':i+1)+'</span><i></i></li>').join('')+'</ol><small>Private workspace<br>on this Mac</small></aside><main class="vx-onboarding-main"><div class="vx-onboarding-copy"><span class="vx-onboarding-eyebrow">'+meta.eyebrow+'</span><h1 id="vx-onboarding-title">'+meta.title+'</h1><p>'+meta.copy+'</p></div><div class="vx-onboarding-content">'+vectorSetupBody(step)+'</div><p class="vx-onboarding-error" role="alert">'+esc(vectorOnboarding.message||'')+'</p><footer><button type="button" class="mp-small-button" id="vx-onboarding-back"'+(step===0?' hidden':'')+'>Back</button><span>'+(step>0&&step<6?step+' / 5':'')+'</span><button type="button" class="mp-action" id="vx-onboarding-next"'+(step===5&&!ready?' disabled':'')+'>'+(step===0?'Set up Vector':step===5?'Continue':step===6?'Start searching':'Continue')+'</button></footer></main></div>';
