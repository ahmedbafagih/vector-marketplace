...
let now=0,documentState='interactive',body={},locationURL=url,pendingURL,checks=0,actions=0;
const chrome={runtime:{onMessage:noop,onStartup:noop,onInstalled:noop,getManifest:()=>({version:'0.2.0'})},storage:{local:{get:async()=>({}),set:async()=>{}}},action:{setBadgeText:async()=>{}},tabs:{onRemoved:noop,get:async()=>({id:1,url,status:'loading',pendingUrl:pendingURL})},scripting:{executeScript:async a=>{checks++;assert.equal(a.injectImmediately,true);return [{result:vm.runInNewContext('('+a.func.toString()+')()',{document:{body,readyState:documentState},location:{href:locationURL}})}]}}};
const c=vm.createContext({chrome,URL,Promise,Error,Date:{now:()=>now},setTimeout:f=>{now+=200;f()}});vm.runInContext(fs.readFileSync(__dirname+'/../Extension/background.js','utf8'),c);
(async()=>{
