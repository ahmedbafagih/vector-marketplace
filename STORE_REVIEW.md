...
3. Open `~/Applications/Vector.app` if it is not already open.
...
3. Open the Vector Marketplace Companion popup.
4. Review the visible data disclosure and choose **Connect Marketplace**.
5. Return to Vector. In the setup guide, choose **Check connections**.
...
The companion reads the visible Marketplace page only after the user chooses **Connect Marketplace**. Vector can then request observations or page actions for the exact Marketplace workflow the user enables. The extension rejects navigation outside Facebook Marketplace and identified Messenger conversations.
