...
final class ChromeBridge {
    ...
    func start() throws {
        ...
        if let existing=Wire.connectSocket(){close(existing);throw VectorError(message:"Another Marketplace AI instance is running. Quit it before opening this copy.")}
        unlink(Wire.path)
        guard var address=Wire.address(Wire.path) else{throw VectorError(message:"Marketplace AI data path is too long.")}
    ...
    private func acceptClient(_ fd:Int32){
        guard client<0 else{Wire.write(fd,["event":"unavailable","error":"Marketplace AI is connected in another Chrome profile. Disconnect it first."]);close(fd);return}
    ...
    func request(_ method:String,_ args:[String:Any]=[:],completion:@escaping(Result<Any,Error>)->Void){
        serial.async{
            guard self.ready,self.client>=0 else{DispatchQueue.main.async{completion(.failure(VectorError(message:"Connect the Marketplace AI extension in Chrome first.")))};return}
