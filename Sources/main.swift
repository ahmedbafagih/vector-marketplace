...
final class App: NSObject, NSApplicationDelegate, WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate, UNUserNotificationCenterDelegate {
    ...
    func userContentController(_ userContentController:WKUserContentController,didReceive message:WKScriptMessage){
        ...
        case "openCompanionStore":guard let value=a["url"] as? String,let url=URL(string:value),url.scheme=="https",url.host=="chromewebstore.google.com",url.path.hasSuffix("/elkhmobmkaiepgennfgabmbolldjlmpb") else{throw VectorError(message:"The Chrome companion listing could not be opened.")};guard let chrome=NSWorkspace.shared.urlForApplication(withBundleIdentifier:"com.google.Chrome") else{throw VectorError(message:"Google Chrome is not installed.")};NSWorkspace.shared.open([url],withApplicationAt:chrome,configuration:NSWorkspace.OpenConfiguration()){_,error in if let error{self.respond(id,.failure(error))}else{self.respond(id,.success(["opened":true]))}}
