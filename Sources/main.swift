...
final class App: NSObject, NSApplicationDelegate, WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate, UNUserNotificationCenterDelegate {
    ...
    func applicationDidFinishLaunching(_ notification:Notification){
        do{
            ...
            window.title="Marketplace AI";window.minSize=NSSize(width:640,height:540);window.contentView=web;window.isReleasedWhenClosed=false;window.center();window.makeKeyAndOrderFront(nil)
            web.loadFileURL(resources.appendingPathComponent("index.html"),allowingReadAccessTo:resources)
            NSApp.activate(ignoringOtherApps:true)
        }catch {let alert=NSAlert();alert.messageText="Marketplace AI could not start";alert.informativeText=error.localizedDescription;alert.runModal();NSApp.terminate(nil)}
    }
    func makeMenu(){
        let main=NSMenu();let appItem=NSMenuItem();let appMenu=NSMenu();appMenu.addItem(withTitle:"About Marketplace AI",action:#selector(about),keyEquivalent:"");appMenu.addItem(.separator());appMenu.addItem(withTitle:"Quit Marketplace AI",action:#selector(NSApplication.terminate(_:)),keyEquivalent:"q");appItem.submenu=appMenu;main.addItem(appItem)
    ...
    @objc func about(){let alert=NSAlert();alert.messageText="Marketplace AI " + (Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "");alert.informativeText="Local Marketplace workspace. Your inventory is stored on this Mac.";alert.runModal()}
    ...
    func userContentController(_ userContentController:WKUserContentController,didReceive message:WKScriptMessage){
        ...
        case "notify":let title=String((a["title"] as? String ?? "Marketplace AI").prefix(80)),body=String((a["body"] as? String ?? "").prefix(240)),key=String((a["key"] as? String ?? UUID().uuidString).prefix(120));let content=UNMutableNotificationContent();content.title=title;content.body=body;content.userInfo=["itemId":a["itemId"] as? Int ?? 0,"section":a["section"] as? String ?? "Needs attention"];UNUserNotificationCenter.current().add(UNNotificationRequest(identifier:key,content:content,trigger:nil));respond(id,.success(["scheduled":true]))
        ...
        case "openCompanionStore":guard let value=a["url"] as? String,let url=URL(string:value),url.scheme=="https",url.host=="chromewebstore.google.com",url.path.hasSuffix("/cfkdpkejcgfgokangnmkoofnmigkkokg") else{throw VectorError(message:"The Chrome companion listing could not be opened.")};guard let chrome=NSWorkspace.shared.urlForApplication(withBundleIdentifier:"com.google.Chrome") else{throw VectorError(message:"Google Chrome is not installed.")};NSWorkspace.shared.open([url],withApplicationAt:chrome,configuration:NSWorkspace.OpenConfiguration()){_,error in if let error{self.respond(id,.failure(error))}else{self.respond(id,.success(["opened":true]))}}
